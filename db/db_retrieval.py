...
# def get_postgres_state_details(conn_data, active_only=False):
#     """Fetches detailed server state including sessions, locks, and prepared transactions."""
#     try:
#         conn = create_postgres_connection(conn_data, application_name="Universal SQL Client")
#         if conn:
#             conn.set_session(readonly=True, autocommit=True)
#             with conn.cursor() as cur:
#                 # 1. Sessions (Activity)
#                 # Columns: PID, User, Application, Client, Backend start, Transaction start, State, Wait event, Blocking PIDs
#                 # Extra (Hidden/Details): Backend Type, Query Start, State Change, SQL Query
#                 session_query = """
#                     SELECT 
#                         pid, 
#                         usename AS "User", 
...
#                 """
#                 if active_only:
#                     session_query = session_query.replace("WHERE", "WHERE state != 'idle' AND state IS NOT NULL AND")
...
#                 # 2. Locks
#                 # Columns: PID, Lock type, Target relation, Page, Tuple, vXID, XID (target), Class, Object ID, vXID (owner), Mode, Granted?
#                 cur.execute("""
#                     SELECT 
#                         pid, 
#                         locktype AS "Lock type", 
#                         relation::regclass AS "Target relation", 
#                         page AS "Page", 
#                         tuple AS "Tuple", 
#                         virtualxid AS "vXID", 
...
#                 # Columns: Name, Owner, XID, Prepared at
#                 cur.execute("""
#                     SELECT 
#                         gid AS "Name", 
#                         owner AS "Owner", 
#                         transaction AS "XID", 
#                         prepared AS "Prepared at"
...
def get_postgres_state_details(conn_data, active_only=False):
    """Fetches detailed server state with dynamic application name based on database and connection name."""
    try:
        db_name = conn_data.get('database', 'unknown_db')
        conn_label = conn_data.get('name', 'unknown_conn')
...
def get_postgres_session_stats(conn_data, current_db_only=False):
    """Fetches high-accuracy PG stats with dynamic application name."""
    try:
        db_name = conn_data.get('database', 'unknown_db')
        conn_label = conn_data.get('name', 'unknown_conn')
        app_name = f"Universal SQL Client - {db_name}"
