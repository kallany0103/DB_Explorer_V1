...
class BaseDialect(default.DefaultDialect):
  ...
  @mtd_logger
  def has_table(self, connection, table_name, schema=None, info_cache={}, **kw):
    """
    :return: bool
    """
    ret = bool(self.get_columns(connection, table_name, schema=schema, info_cache=info_cache))
    return ret

  @mtd_logger
  def get_foreign_keys(self, connection, table_name, schema=None, info_cache={}, **kw):
    """
    ...
    foreign_keys = [
      {
        ...
        "referred_schema": row[7],
        "referred_table": row[8],
        "referred_columns": [row[9]],
        "catalog_name": row[0],
        "schema_name": row[1],
        "table_name": row[2],
        "column_name": row[3],
        "primary_key_name": row[4],
        "referred_catalog": row[6],
        "foreign_key_type": row[10],
  ...
  @mtd_logger
  def get_indexes(self, connection, table_name, schema=None, info_cache={}, **kw):
    """
    ...
    indexes = [
      {
        "name": row[0],
        "column_names": [row[1]],
        "unique": self._get_value_as_bool(row[2]),
      } for row in rp.fetchall()
    ]
  ...
  @mtd_logger
  def get_schema_names(self, connection, info_cache={}, **kw):
  ...
  @mtd_logger
  def get_table_names(self, connection, schema=None, info_cache={}, **kw):
    """
  ...
  @mtd_logger
  def get_view_names(self, connection, schema=None, info_cache={}, **kw):
    """
  ...
  @mtd_logger
  def get_columns(self, connection, table_name, schema=None, info_cache={}, **kw):
    """
  ...
  @mtd_logger
  def get_pk_constraint(self, connection, table_name, schema=None, info_cache={}, **kw):
    """
