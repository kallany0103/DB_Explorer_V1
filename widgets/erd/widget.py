...
from PySide6.QtGui import QAction, QTransform, QPixmap, QPainter, QFont, QColor, QUndoStack, QPdfWriter, QPageSize, QPageLayout
from PySide6.QtCore import Qt, QSize, QSizeF, QMarginsF, QRectF, QTimer, QPointF
    ...
    def save_as_image(self, ext="png"):
        ...
        try:
            if file_path.endswith('.svg'):
                # QSvgGenerator doesn't support drawPixmap in most SVG viewers, so
                # render to high-res pixmap and embed as base64 PNG data URI instead
                import base64
                ...
                px_painter.setRenderHint(QPainter.RenderHint.Antialiasing)
                px_painter.setRenderHint(QPainter.RenderHint.SmoothPixmapTransform)
                px_painter.setRenderHint(QPainter.RenderHint.TextAntialiasing)
                px_painter.scale(scale_factor, scale_factor)
                self.scene.render(px_painter, QRectF(0, 0, items_rect.width(), items_rect.height()), items_rect)
                px_painter.end()
                ...
                # Render to high-res pixmap first (avoids QPdfWriter vector scaling bugs)
                scale_factor = 3.0
                w = int(items_rect.width() * scale_factor)
                h = int(items_rect.height() * scale_factor)
                ...
                # Embed the pixmap into a PDF page sized to match the diagram
                base_dpi = 96
